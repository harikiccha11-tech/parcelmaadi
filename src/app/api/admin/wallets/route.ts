...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (status && "status" in db.wallet.fields) where.status = status;
    

    const items = await db.wallet.findMany({
      where,
      orderBy: { id: "desc" },
      take: limit,
    });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const allowed = ["holderType","holderId"];
    const data: any = {};
    for (const k of allowed) {
      if (k in body) data[k] = body[k];
    }
    const item = await db.wallet.create({ data });
